...
probe = LinearProbe(config.hidden_size, config.vocab_size).to(device, torch.bfloat16)

# 4. Data collator and loader
data_collator = HaploSimpleDataCollator(subsample=32, mlm_probability=0.0, whole_snp_mask_probability=0.75)
...
for epoch in range(EPOCHS):
    ...
    for batch in pbar:
        ...
        hidden_outputs, _ = model.roberta(input_ids, distances)

        logits = probe(hidden_outputs[0])  # (batch, haps, snps, vocab)
