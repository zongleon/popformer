...
class SweepNet1DDet(torch.nn.Module):
    ...
    def __init__(self, H, W, outputs, channels):
        ...
        out_conv1 = self.compute_out_shape(
            (H, W), (conv1_kernel, conv1_kernel), (conv1_stride, conv1_stride)
        )
        out_pool1 = self.compute_out_shape(
            out_conv1, (pool1_kernel, pool1_kernel), (pool1_stride, pool1_stride)
        )
        out_conv2 = self.compute_out_shape(
            out_pool1, (conv2_kernel, conv2_kernel), (conv2_stride, conv2_stride)
        )
        out_pool2 = self.compute_out_shape(
            out_conv2, (pool2_kernel, pool2_kernel), (pool2_stride, pool2_stride)
        )
        out_conv3 = self.compute_out_shape(
            out_pool2, (conv3_kernel, conv3_kernel), (conv3_stride, conv3_stride)
        )
        out_pool3 = self.compute_out_shape(
            out_conv3, (pool3_kernel, pool3_kernel), (pool3_stride, pool3_stride)
        )
        out_conv4 = self.compute_out_shape(
            (out_pool3[0], out_pool3[1]),
            (conv4_kernel, conv4_kernel),
            (conv4_stride, conv4_stride),
        )
        out_conv5 = self.compute_out_shape(
            (out_conv4[0], out_conv4[1]),
            (conv5_kernel, conv5_kernel),
            (conv5_stride, conv5_stride),
        )
        out_conv6 = self.compute_out_shape(
            (out_conv5[0], out_conv5[1]),
            (conv5_kernel, conv5_kernel),
            (conv5_stride, conv5_stride),
        )
